...
import { getAllCompanyVisits, addCompanyVisit, deleteCompanyVisit } from '../utils/api';
...
function AdminCompanyVisits() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const rolesArray = formData.rolesOffered.split(',').map(role => role.trim());
      ...
      alert('Error adding company visit: ' + (error.response?.data?.message || 'Unknown error'));
    } finally {
      setLoading(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-linear-to-br from-gray-50 via-purple-50 to-gray-100">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <p className="text-gray-600 mt-1">Add and manage company recruitment information</p>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-6">
              ...
              <div>
                ...
                <input
                  ...
                  className="w-full px-4 py-3 rounded-xl border-2 border-gray-200 focus:border-blue-500 focus:ring-2 focus:ring-blue-200 outline-none transition"
                />
                <p className="text-sm text-gray-500 mt-1">Separate multiple roles with commas</p>
              </div>
              ...
              >
                {loading ? 'Adding...' : '✓ Add Company Visit'}
              </button>
            </form>
          </div>
        )}
        ...
        {/* Company Visits List */}
        <div>
          <h3 className="text-2xl font-bold text-gray-800 mb-6">📊 All Company Visits ({companyVisits.length})</h3>
          
          {companyVisits.length === 0 ? (
            <div className="bg-white rounded-2xl shadow-md p-12 text-center border border-gray-100">
              <div className="text-6xl mb-4">🏢</div>
              <p className="text-gray-600 text-lg">No company visits added yet.</p>
              <p className="text-gray-500 mt-2">Click the button above to add your first company!</p>
            </div>
          ) : (
                ...
                >
                  <div className="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4">
                    <div className="flex-1">
                      <h4 className="text-2xl font-bold text-gray-800 mb-2">{visit.companyName}</h4>
                      <div className="space-y-3">
                        <p className="text-gray-600">
                          <strong>Batch:</strong> {visit.batch}
                        </p>
                        <p className="text-gray-600">
                          <strong>Package Range:</strong> <span className="text-green-600 font-bold">₹{visit.packageRange.min} - {visit.packageRange.max} LPA</span>
                        </p>
                        <div>
