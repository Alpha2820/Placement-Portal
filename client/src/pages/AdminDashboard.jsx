...
import { getPendingPlacements, approvePlacement, rejectPlacement } from '../utils/api';
...
function AdminDashboard() {
  ...
  const handleApprove = async (id) => {
    if (!window.confirm('Are you sure you want to approve this placement?')) return;
  ...
  const handleReject = async (id) => {
    if (!window.confirm('Are you sure you want to reject this placement?')) return;
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
              ...
              <p className="text-gray-600 mt-1">Manage pending placement approvals</p>
            </div>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8 bg-linear-to-r from-purple-600 to-pink-600 rounded-2xl shadow-xl p-6 text-white">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-purple-100 text-sm font-semibold uppercase tracking-wide">Pending Approvals</p>
        ...
        {/* Pending Placements */}
        <h3 className="text-2xl font-bold text-gray-800 mb-6">📋 Pending Placements</h3>

        {placements.length === 0 ? (
          <div className="bg-white rounded-2xl shadow-md p-12 text-center border border-gray-100">
            <div className="text-6xl mb-4">✅</div>
            <p className="text-gray-600 text-lg font-semibold">All caught up!</p>
            <p className="text-gray-500 mt-2">No pending placements to review at the moment.</p>
          </div>
        ) : (
                  ...
                  <div className="flex-1 space-y-4">
                    ...
                    <div className="grid md:grid-cols-2 gap-4">
                      <div className="space-y-2">
                        <p className="text-gray-600">
                          <strong className="text-gray-800">Student:</strong> {placement.studentName}
                        </p>
                        <p className="text-gray-600">
                          <strong className="text-gray-800">Roll Number:</strong> {placement.userId?.rollNumber}
                        </p>
                        <p className="text-gray-600">
                          <strong className="text-gray-800">Email:</strong> {placement.userId?.email}
                        </p>
                      </div>
                      ...
                      <div className="space-y-2">
                        <p className="text-gray-600">
                          ...
                          <span className="text-green-600 font-bold text-xl ml-2">₹{placement.package} LPA</span>
                        </p>
                        <p className="text-gray-600">
                          <strong className="text-gray-800">Batch:</strong> {placement.batch}
                        </p>
                        <p className="text-gray-600">
                          <strong className="text-gray-800">Submitted:</strong> {new Date(placement.createdAt).toLocaleDateString('en-IN')}
                        </p>
                      </div>
                    </div>
                    ...
                    {placement.interviewExperience && (
                      <div className="bg-gray-50 rounded-xl p-4 border border-gray-200">
                        <strong className="text-gray-800 block mb-2">💬 Interview Experience:</strong>
                        <p className="text-gray-700 leading-relaxed">{placement.interviewExperience}</p>
                      </div>
                    )}
